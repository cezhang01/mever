...
def parse_args():
    ...
    parser.add_argument('-ls', '--log_steps', type=int, default=1)
    parser.add_argument('-lr', '--learning_rate', type=float, default=1e-5)
    parser.add_argument('-ms', '--minibatch_size', type=int, default=16)  # may need to change
    parser.add_argument('-ml', '--max_text_length', type=int, default=128)  # may need to change
    parser.add_argument('-ni', '--num_sampled_images', type=int, default=1)  # may need to change
    parser.add_argument('-s', '--save_retrieval_result', type=bool, default=False)
...
def load_data(args):
    ...
    training_data = Data(data_center, 'train')
    test_data_claim, test_data_evid = Data(data_center, 'test_claim'), Data(data_center, 'test_evid')
    ...
    test_sampler_claim, test_sampler_evid = SequentialSampler(test_data_claim), SequentialSampler(test_data_evid)
    training_loader = DataLoader(training_data, batch_size=args.minibatch_size, sampler=training_sampler)
    test_loader_claim = DataLoader(test_data_claim, batch_size=args.minibatch_size, sampler=test_sampler_claim)
    test_loader_evid = DataLoader(test_data_evid, batch_size=args.minibatch_size, sampler=test_sampler_evid)

    return data_center, training_loader, test_loader_claim, test_loader_evid
...
def train(args):
    ...
    if args.local_rank in [-1, 0]:
        print('Loading data...')
    data_center, training_loader, test_loader_claim, test_loader_evid = load_data(args)
    ...
    for epoch_id in range(1, args.num_epochs + 1):
        ...
        for batch_id, batch in tqdm(enumerate(training_loader), total=len(training_loader)):
            claim_evid_pairs = batch
            optimizer.zero_grad()
            res = ddp_model(claim_evid_pairs, data_center, mode='train')
            loss = res[0]
            loss.backward()
            optimizer.step()
            with torch.no_grad():
        ...
        if epoch_id % args.log_steps == 0 and args.local_rank in [-1, 0]:
            ...
            #     os.makedirs('./ckpt')
            # torch.save(model.state_dict(), './ckpt/' + args.dataset_name + '_retrieval_' + str(epoch_id) + '.pt')
            test(model, data_center, test_loader_claim, test_loader_evid, args)
        if args.distributed_training:
            torch.distributed.barrier()
    if args.distributed_training:
        cleanup()
...
def test(model, data_center, test_loader_claim, test_loader_evid, args):

    model.eval()
    claim_emb, evid_emb = [], []
    for batch_id, batch in tqdm(enumerate(test_loader_claim), total=len(test_loader_claim)):
        claim_evid_pairs = batch
        res = model(claim_evid_pairs, data_center, mode='test_claim')
        claim_emb_tmp = res[1].detach().cpu().numpy().tolist()
        claim_emb.extend(claim_emb_tmp)
    for batch_id, batch in tqdm(enumerate(test_loader_evid), total=len(test_loader_evid)):
        claim_evid_pairs = batch
        res = model(claim_evid_pairs, data_center, mode='test_evid')
        evid_emb_tmp = res[2].detach().cpu().numpy().tolist()
        evid_emb.extend(evid_emb_tmp)
    claim_emb, evid_emb = np.array(claim_emb), np.array(evid_emb)
    test_claim_emb = claim_emb[data_center.test_claim_ids]

    evidence_retrieval(test_claim_emb, evid_emb, data_center.test_claim_ids, data_center.test_claim_evid_pairs)
    if args.save_retrieval_result:
        save_retrieval_result(data_center, claim_emb, evid_emb, args, k=5)
...
def main(args):
    ...
    if args.mode == 'train':
        ...
        data_center, training_loader, test_loader_claim, test_loader_evid = load_data(args)
        model = Model(args, data_center).to(args.device)
        ckpt = torch.load('./ckpt/' + args.dataset_name + '_retrieval_50.pt', map_location='cpu')

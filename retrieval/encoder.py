...
class EncoderLayer(nn.Module):
    ...
    def forward(self, bert, bert_hidden_states, attention_mask, vit, vit_hidden_states, mode):
        ...
        for layer_id in range(self.num_hidden_layers):
            ...
            if layer_id > 0:
                ...
                bert_cls_emb_reshape = torch.reshape(bert_cls_emb, [-1, 1, self.bert_hidden_size])  # self-loop
...
class Encoder(nn.Module):
    ...
    def __init__(self, bert_config, vit_config, args):
        ...
        self.linear_layers = nn.ModuleList([nn.Linear(self.bert_hidden_size + self.vit_hidden_size, self.bert_hidden_size),
                                            nn.Linear(self.bert_hidden_size, self.bert_hidden_size)])

    def forward(self, bert, input_ids, attention_mask, vit, pixel_values, mode):
        ...
        bert_cls_emb = bert_hidden_states[:, 0, :]

        if len(all_vit_hidden_states) > 0:
            vit_hidden_states = all_vit_hidden_states[-1]
            vit_hidden_states = vit_hidden_states[:, 2:, :]
            vit_cls_emb = vit_hidden_states[:, 0, :]

            vit_cls_emb = torch.reshape(vit_cls_emb, [num_texts, self.num_sampled_images, self.vit_hidden_size])
            vit_cls_emb_agg = torch.mean(vit_cls_emb, dim=1)
            cls_emb = self.linear_layers[0](torch.concat([bert_cls_emb, vit_cls_emb_agg], dim=-1))
        else:
            cls_emb = self.linear_layers[1](bert_cls_emb)
        # cls_emb = self.linear_layer[0](cls_emb)
